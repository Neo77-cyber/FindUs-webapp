...
for i in range(5):
    ...
    craftsman = CraftsmanProfile.objects.create(
        ...
        license_number=(
            f"LIC-00{random.randint(1000, 9999)}"
...
for craftsman in craftsmen:
    ...
    for j in range(1, num_services + 1):
        ...
        if price_type == "hourly":
            ...
            min_hours = (
                f"{random.randint(1, 4)} hours" if random.choice([True, False]) else ""
            )
        else:  # fixed price
            hourly_rate = None
        ...
        print(
            f"     Price: {'$' + str(service.hourly_rate) + '/hour' if service.hourly_rate else '$' + str(service.fixed_price) + ' fixed'}"
        )
...
for i in range(5, 10):
    ...
    customer = CustomerProfile.objects.create(
        user_profile=user_profile, phone=f"+39 322 555 66{i-4:02d}"
    )
