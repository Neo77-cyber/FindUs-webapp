...
from ..services import CraftsmanServiceManager, CacheManager, delete_service_safe
...
def craftsman_dashboard(request):
    ...
    try:
        ...
        services_qs = CraftsmanServiceManager.get_craftsman_services(
            craftsman_profile, service_filter
        )
...
def delete_service(request):
    ...
    try:
        ...
        if not service_id:
            return JsonResponse(
                {"success": False, "error": "Service ID is required"}, status=400
            )
    ...
    if request.method == "POST":
        try:
            ...
            if service.price_type == "hourly":
                ...
                service.hourly_rate = (
                    raw_hourly if raw_hourly and raw_hourly.strip() else None
                )
                service.fixed_price = None
            else:
                raw_fixed = request.POST.get("fixed_price")
                service.fixed_price = (
                    raw_fixed if raw_fixed and raw_fixed.strip() else None
                )
            ...
            return JsonResponse(
                {
                    ...
                    "service": {
                        ...
                        "hourly_rate": (
                            str(service.hourly_rate) if service.hourly_rate else ""
                        ),
                        "fixed_price": (
                            str(service.fixed_price) if service.fixed_price else ""
                        ),
...
def check_boost_status(request, service_id):
    ...
    try:
        ...
        return JsonResponse(
            {"has_pending_boost": has_pending_boost, "service_id": service_id}
        )

    except Exception as e:
        logger.error(f"Error checking boost status: {e}", exc_info=True)
        return JsonResponse({"has_pending_boost": False, "error": str(e)})
...
def boost_service(request):
    ...
    try:
        ...
        if not payment_proof:
            return JsonResponse(
                {"success": False, "error": "Please upload payment proof"}, status=400
            )
        ...
        logger.info(
            f"Boost request created: {boost_request.id} for service {service.id}"
        )
