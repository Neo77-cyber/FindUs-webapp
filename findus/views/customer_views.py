    ...
    try:
        ...
        if not is_htmx:
            ...
            if cached_context:
                ...
                template = "partials/dashboard_results.html" if is_htmx else "customer_dashboard.html"
        ...
        if is_infinite_scroll:
            ...
            html = render_to_string('partials/dashboard_results.html', {
                'services': page_obj,
                'Service': Service,
            })
...
def service_detail(request, service_slug):
    ...
    if cached_context and cached_context.get('service') and not messages.get_messages(request):
...
def submit_review(request, service_slug):
    ...
    cache_key = f"service_detail_{service_slug}"
    CacheManager.set_cached_context(cache_key, None, 1)  # Clear by setting None with short expiry
